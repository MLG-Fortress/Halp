...
public class BukkitInfoBuilder {
    ...
    public String Build(String k) {
        ...
        b.append("Server Log: " + NSAConnector.upload(new File(Bukkit.getPluginManager().getPlugin("Halp").getDataFolder(),"../../logs/latest.log"), HalpBukkit.service)).append("\n");
        b.append("server.properties: " + NSAConnector.upload(new File(Bukkit.getPluginManager().getPlugin("Halp").getDataFolder(),"../../server.properties"), HalpBukkit.service)).append("\n");
        b.append("spigot.yml: " + NSAConnector.upload(new File(Bukkit.getPluginManager().getPlugin("Halp").getDataFolder(),"../../spigot.yml"), HalpBukkit.service)).append("\n");
        b.append("paper.yml: " + NSAConnector.upload(new File(Bukkit.getPluginManager().getPlugin("Halp").getDataFolder(),"../../paper.yml"), HalpBukkit.service)).append("\n\n");
            ...
            if(k.equalsIgnoreCase("k")) { //probably a bad idea.
                try {
                    Files.walk(Paths.get(p.getDataFolder().getCanonicalPath()))
                            ...
                            .map(Path::toFile)
                            .collect(Collectors.toList()).stream().filter(file -> file.getName().endsWith(".yml") || file.getName().endsWith(".log")).forEach(file -> b.append(file.getName() + ": " + NSAConnector.upload(file, HalpBukkit.service)).append("\n     "));
                } catch (IOException e) {
                    b.append("No files" + "\n  ");
                }
                b.append("\n   ");
